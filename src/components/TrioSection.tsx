...
export const TrioSection = () => {
  return (
    <div id="tw-scope" className="py-40 px-24 dark:bg-[#1F1F21]">
      <div className="max-w-360 mx-auto grid grid-cols-3">
        {ITEMS.map((item) => (
          <div
            key={item.title}
            className="flex flex-col items-center text-center px-6.5"
          >
            <div className="bg-[#00543D]/5 dark:bg-white/5 rounded-full mb-14 aspect-square flex items-center justify-center">
              <img
                src={item.imageURL}
                alt={item.title}
            ...
            <p className="text-[#9C9C9C] dark:text-white/50 tracking-[-0.32px] text-base font-normal leading-6">
              {item.description}
            </p>
          </div>
        ))}
